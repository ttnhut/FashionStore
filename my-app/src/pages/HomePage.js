...
const HomePage = () => {
  ...
  const changePage = (pageNumber=0,pageSize=6)=>{
    ...
    loadAddClothes(pageNumber,pageSize).then(res=>{
      console.log(res)
      setClothesContent({
        content:[...clothesContent.content,...res.content],
      ...
      <main className=" min-h-[60.5vh]">
        ...
        <section className="p-8">
          <div className="container xl:w-[1300px] lg:w-[1044px] md:w-[788px]  relative px-4 ml-auto mr-auto">
            ...
            <Slider {...settings}>
              {clothesContent?.content?.length >= 0 &&
          ...
          <div className="container xl:w-[1300px] lg:w-[1044px] md:w-[788px] py-[15px] relative  mr-auto ml-auto">
            ...
            <div className="tap-reuslt">
              <div className="block ml-6">
                <div className="-ml-[15px] grid xl:grid-cols-1 lg:grid-cols-1 md:grid-cols-1 sm:grid-cols-1 gap-[0.5px]">
                  ...
                  {/* {clothesContent?.content?.length > 0 &&
...

                    {
                      clothesContent && (
                        <InfiniteScroll
                      dataLength={clothesContent?.content?.length}
                      next={changePageInfinite}
                      hasMore={!clothesContent?.lastPage}
                      loader={<h4>Loading...</h4>}
                      endMessage={<h4>Đã hết dữ liệu</h4>}
                    >
                      {
                    clothesContent.content.map((item) => (
                      <Link to={`/clothes/${item.id}`}>
    ...
    <div className="flex items-center justify-center bg-white">
      <div className="p-2 ">
        <div className="bg-white rounded-lg shadow hover:shadow-md">
          <div className="relative max-w-[1000px] overflow-hidden rounded-t cursor-pointer group ">
            <img
              src={image}
              alt=""
              className="object-cover w-[1000px] h-[600px] group-hover:scale-105 "
            />
